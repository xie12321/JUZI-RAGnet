...
def compile_knowledge(query: str, context: str = "", categories: Optional[List[str]] = None, llm=None) -> str:
    ...
    results = retriever.hybrid_search_with_graph(query, categories=categories, top_k=10, expand_neighbors=False)
    ...
    prompt = f"""你现在处于【知识提炼】阶段。

    在这个阶段，你需要做的是：
    - 仔细阅读以下原始文档，这些文档是从知识库中检索出来的。
    - 围绕用户的问题，从中提炼出最核心、最直接相关的知识点。
    - 严格基于原文，不要添加原文中没有的信息，也不要猜测。
    - 如果有多个文档，请合并去重，避免重复输出相同的内容。
    - 将提炼出的知识点以简洁、清晰的条目列出，每条一句话。

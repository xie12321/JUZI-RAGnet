...
from config import get_wiki_root, CHROMA_PATH, CHROMA_COLLECTION_NAME, EMBED_MODEL, HYBRID_TOP_K, BM25_WEIGHT, VECTOR_WEIGHT
...
class RequestsEmbeddings:
    """使用 requests 直连 Ollama embed API，替代不稳定的 ollama Python 库"""
...
class WikiRetriever:
    def __init__(self, rebuild: bool = False):
        self.embeddings = RequestsEmbeddings(model=EMBED_MODEL, base_url="http://localhost:11434")
    ...
    def _build_index(self):
        all_md_files = list(get_wiki_root().rglob("*.md"))
        all_chunks = []
        for md_file in all_md_files:
            if '.obsidian' in md_file.parts:
                continue
            # ★ 只对经验库进行向量化，推理库和记忆库不参与
            path_str = str(md_file)
            if 'Reasoning' in path_str or 'reasoning' in path_str:
                continue
            if 'Memory' in path_str or 'memory' in path_str:
                continue
        ...
        logger.info(f"Built index with {len(all_chunks)} chunks (经验库 only)")
    ...
    def hybrid_search_with_graph(self, query: str, categories: List[str] = None, top_k: int = 10,
                                 expand_neighbors: bool = False, enable_tag_aggregation: bool = False) -> List[Dict]:
        """
        混合检索 + 知识图谱增强，支持按类别过滤
    ...
    def get_all_skill_titles(self) -> List[str]:
        """获取所有可用的 Skill 文档标题（从推理库和记忆库）"""
        from config import get_wiki_root, REASONING_DIR, MEMORY_DIR
        import frontmatter as fm
        titles = []
        for subdir_name in [REASONING_DIR, MEMORY_DIR]:
            subdir = get_wiki_root() / subdir_name
    ...
    def get_document_by_title(self, title: str) -> Optional[str]:
        """通过文档标题精确获取文档全文"""
        import frontmatter as fm
        try:
            ...
            logger.warning(f"获取文档失败 {title}: {e}")
        return None

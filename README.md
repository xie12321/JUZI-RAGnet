...
| **🧠 五层记忆** | 短期（会话内）、中期（跨会话持久化）、长期（向量库）、经验库（成功/失败案例）、推理库（逻辑学知识）。 |
| **🛠️ 工具调用** | 内置联网搜索、长期记忆检索；支持 OpenAI 兼容的外部工具扩展。 |
| **🎯 结构化输出** | 强制输出 Pydantic 模型，配合规划验证器确保计划合法性。 |
| **🌐 OpenAI 兼容 API** | 无缝接入 OpenClaw、Chatbox 等任何支持该标准的客户端。 |
| **🧹 智能记忆整理** | LLM 驱动的自动整理：提取重要信息存入长期记忆，不重要的内容压缩为摘要。 |
| **📖 学习能力** | 通过在对话中不断丰富的经验库和推理库，让LLM越来越聪明。 |
